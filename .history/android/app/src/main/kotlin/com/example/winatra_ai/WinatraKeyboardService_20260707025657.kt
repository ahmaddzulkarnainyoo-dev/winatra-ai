...
class WinatraKeyboardService : InputMethodService() {
    ...
    companion object {
        ...
        // Hanya DeepSeek — Groq dihapus total
        ...
        const val COLOR_KEY_BG        = "#2C2C2E"
        const val COLOR_KEY_FUNC      = "#3D3D3F"
        const val COLOR_KEY_ACTION    = "#0A84FF"
        const val COLOR_KEY_PRESSED   = "#4A4A4C"
        const val COLOR_KEY_TEXT      = "#FFFFFF"
        const val COLOR_KB_BG         = "#131315"
    ...
    private fun buildKeyButton(ctx: Context, label: String): TextView {
        ...
        val (displayText, bgColor, fontSize) = when (label) {
            "⇧"   -> Triple("⇧", if (isShift && !isCaps) "#0A84FF" else COLOR_KEY_FUNC, 20f)
            "⇪"   -> Triple("⇪", COLOR_KEY_ACTION, 20f)
            "⌫"   -> Triple("⌫", COLOR_KEY_FUNC, 20f)
            "space" -> Triple("", COLOR_KEY_FUNC, 14f)
            "↩"   -> Triple("↩", COLOR_KEY_ACTION, 20f)
            "?123" -> Triple("?123", COLOR_KEY_FUNC, 13f)
            "ABC"  -> Triple("ABC", COLOR_KEY_FUNC, 13f)
            else  -> Triple(label, COLOR_KEY_BG, 20f)
        }
        ...
        scope.launch {
            try {
                ...
                if (FlutterBridge.isReady()) {
                    try {
                        result = FlutterBridge.getAnswer(question)
                ...
                if (!usedBridge) {
                    ...
                    result = callAIWithFallback(question, mode)
                }
