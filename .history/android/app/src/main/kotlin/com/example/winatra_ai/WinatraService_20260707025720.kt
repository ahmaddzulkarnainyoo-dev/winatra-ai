...
class WinatraService : Service() {
    ...
    companion object {
        ...
        // Hanya DeepSeek endpoints — Groq dihapus total
        private val API_ENDPOINTS = listOf(
            ApiEndpoint("sk-3f82fc1304a1486b99ae94d4bb9b59b4", "https://api.deepseek.com/v1", "DeepSeek"),
        )
    ...
    // ---------- DEEPSEEK API (hanya DeepSeek, tanpa Groq) ----------
    private suspend fun callAIWithFallback(question: String, mode: String): String {
        val deepseekEndpoint = API_ENDPOINTS.first { it.type == "DeepSeek" }
        Log.i(TAG, "Calling DeepSeek API (Q: ${question.take(50)})")
        
        // Coba hingga 3 kali jika gagal
        var attemptCount = 0
        val maxAttempts = 3
        while (attemptCount < maxAttempts) {
            attemptCount++
            val result = performApiRequest(deepseekEndpoint, question, mode)
            if (result != null && !result.startsWith("Error:")) {
                Log.i(TAG, "✓ DeepSeek success after $attemptCount attempts")
                return result
            } else {
                Log.w(TAG, "✗ DeepSeek attempt $attemptCount failed: ${result?.take(30)}")
            }
        }
        Log.e(TAG, "✗ DeepSeek failed after $maxAttempts attempts!")
    ...
    private suspend fun performApiRequest(endpoint: ApiEndpoint, question: String, mode: String): String? {
        ...
        return try {
            Log.d(TAG, "→ Calling DeepSeek at $url (specialization=$specialization)")
            ...
            if (response.isSuccessful) {
                ...
                Log.d(TAG, "← Got response from DeepSeek: ${answer.take(50)}...")
                answer
            } else {
                ...
                Log.w(TAG, "✗ DeepSeek returned ${response.code}: $errorMsg. Body: ${body.take(100)}")
                "Error: $errorMsg"
            }
        } catch (e: java.net.SocketTimeoutException) {
            Log.w(TAG, "✗ DeepSeek timeout: ${e.message}")
            "Error: timeout"
        } catch (e: java.io.IOException) {
            Log.w(TAG, "✗ DeepSeek network error: ${e.message}")
            "Error: network"
        } catch (e: Exception) {
            Log.e(TAG, "✗ DeepSeek exception: ${e.message}", e)
            "Error: ${e.message}"
        }
    }
    ...
    private suspend fun callExplanationWithFallback(question: String, answer: String): String {
        val deepseekEndpoint = API_ENDPOINTS.first { it.type == "DeepSeek" }
        return performExplanationRequest(deepseekEndpoint, question, answer) ?: "Error: All API keys failed for explanation."
    }
    ...
    private fun processClipboardResult(question: String, modeType: String = "answer") {
        ...
        scope.launch {
            ...
            withContext(Dispatchers.Main) {
                // Cancel thinking notification dan tampilkan hasil
                (getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager).cancel(RESULT_NOTIF_ID + 3)
                ...
                if (modeType == "discussion") {
                    ...
                    if (getMode() == "PG") {
                        ...
                        showResultNotification("Jawaban dari Winatra", answer)
                    }
                }
            }
        }
    ...
    private fun showThinkingNotification() {
        val builder = NotificationCompat.Builder(this, RESULT_CHANNEL_ID)
            .setContentTitle("Winatra AI")
            .setContentText("Berpikir...")
            .setStyle(NotificationCompat.BigTextStyle().bigText("Menganalisis pertanyaan Anda..."))
            .setSmallIcon(android.R.drawable.ic_menu_search)
            .setOngoing(true)
            .setAutoCancel(false)
        (getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager).notify(RESULT_NOTIF_ID + 3, builder.build())
    }
