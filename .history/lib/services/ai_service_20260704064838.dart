...
/// Unified AI service that works both online (Groq API) and offline (local LLM).
class AIService {
...
Jawab langsung pertanyaannya tanpa basa-basi.''' ;
  ...
  /// Chat using Groq API (online)
  Future<String> _chatOnline(String message, {String? systemPrompt}) async {
    try {
      final remoteConfig = RemoteConfigService();
      final apiKeysStr = remoteConfig.getGroqApiKeys();
      
      if (apiKeysStr.isEmpty) {
        throw Exception('API key tidak tersedia. Periksa koneksi internet atau hubungi admin.');
      }

      // Parse API keys (comma-separated, pick first)
      final keys = apiKeysStr.split(',').map((k) => k.trim()).where((k) => k.isNotEmpty).toList();
      if (keys.isEmpty) {
